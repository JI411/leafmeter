...
def updateRoi(roi):
   ...
   # print(roi.pos(), roi.size(), roi.getAffineSliceParams(im1.image, im1))
    ...
    ref = lab[int(round(y)), int(round(x))]
    da, db = roi.size().x()/2, roi.size().y()/2
    #mask = (lab[:,:,_L] > ref[_L]-5) & (lab[:,:,_L] < ref[_L]+5) & \
    mask = (lab[:,:,_a] > ref[_a]-da) & (lab[:,:,_a] < ref[_a]+da) & \
           (lab[:,:,_b] > ref[_b]-db) & (lab[:,:,_b] < ref[_b]+db)
...
rois = []
rois.append(pg.TestROI([0,  0], [20, 20], maxBounds=QtCore.QRectF(-10, -10, 230, 230), pen=(0,9)))
